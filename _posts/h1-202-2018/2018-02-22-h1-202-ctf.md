...
Because most of you already know some of the solutions for this CTF,
I've seperated this writeup into each individual challenge :

- [Plain text flag (Mobile)](#plaintext-flag)
- [In ur db? oh no! (Web)](#)
- [Encrypted flag (Encrypted Flag)](#encrypted-flag)
- [Calling out Foul play (Web)](#calling-out-foul-play)
- [/admin (Web)](#)
- [Go-ing down the rabbit hole (Reverse)](#)
...
This will start our emulator and install a `CandidateVote`. Let's run it :
...
`jadx` gives us a UI showing the available packages as well
...
for the various challenge solutions. Here's the notes I've gathered
during the event :
...
Also, we have a list of endpoints that we can test based on the interfaces `e` and
`d` :
...
I have not reversed the `@k` and `@f`, but we can make assumptions as to what
...
So there's XML parsing going on! The next thing that comes to mind is XXE.
Sending out a simple `<?xml>`, you'll notice that the response is escaped
this time with : `&lt;?xml`. This seemed to happen when a `<` character
was next to a special character.
